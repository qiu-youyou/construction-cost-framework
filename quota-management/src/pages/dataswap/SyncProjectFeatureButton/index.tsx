 ...
 * @LastEditTime: 2024-04-18 16:00:32
 * @Description: 同步项目特征
 */
...
import { dataswapSyncProjectFeature } from '../services';
...
export default (props: DataswapProps) => {
  ...
  const onOk = async () => {
    /** 当前库ID */
    setLoading(true);
    const res = await dataswapSyncProjectFeature();
    setLoading(false);
  ...
  const triggerBtn = (
    <Button onClick={handleOnClick} className="BorderButtonLime" loading={loading}>
      <RetweetOutlined /> {props?.buttonText || '同步项目特征'}
    </Button>
  );
