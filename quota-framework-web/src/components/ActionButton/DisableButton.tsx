 ...
 * @LastEditTime: 2023-12-27 10:18:07
 * @Description: 禁用按钮
 */
...
const DisableButton = (prop: ActionButtonProps) => {
  /** Props */
    ...
    if (!!dataLength) {
      modal.confirm({
        ...
        async onOk() {
          ...
          const res = await props.onSubmit({ ids: ids, billStatus: 4 }, props?.current, props?.selections);
    ...
    modal.confirm({
      ...
      async onOk() {
        ...
        const res = await props.onSubmit(
          { ids: [props.current[props.idsKey || '']], billStatus: 4 },
          props?.current,
          props?.selections,
        );
        handleSubmitFinish(res);
  ...
  const ButtonDom = !!props.trigger ? (
    ...
    <Button className="DisableButton" disabled={props?.disabled} onClick={onClick}>
      <LockOutlined />
      {props.buttonText}
    </Button>
  );
